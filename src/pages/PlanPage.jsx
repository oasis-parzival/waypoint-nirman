...
import GearRequestForm from '../components/plan/GearRequestForm';
import GearRequestList from '../components/plan/GearRequestList';
import { getGearRequests } from '../services/gearService';
...
const PlanPage = () => {
  ...
  useEffect(() => {
    // Initial fetch of gear requests
    const fetchRequests = async () => {
      const data = await getGearRequests();
      setGearRequests(data);
    };
    fetchRequests();
  }, []);

  const handleGearRequestCreated = (newRequest) => {
    setGearRequests(prev => [newRequest, ...prev]);
  };
  ...
  return (
    <div className="relative min-h-screen selection:bg-primary selection:text-on-primary font-body">
      ...
      <main className="relative z-10 pt-28 pb-24 px-4 md:px-6 max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-12 gap-8 md:gap-16">
        {/* FORM SIDE */}
        <section className="lg:col-span-5 space-y-8 md:space-y-12">
          ...
          {/* NEW GEAR REQUEST SECTION */}
          <div className="space-y-8 animate-in fade-in slide-in-from-left-4 duration-1000 delay-300">
            <GearRequestForm 
              trekName={formData.trekName} 
              onRequestCreated={handleGearRequestCreated} 
            />
            
            <GearRequestList requests={gearRequests} />
          </div>
        </section>
