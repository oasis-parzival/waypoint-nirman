...
const Header = ({ user, onLoginClick }) => {
  ...
  const navLinks = [
    ...
    { name: 'Admin', path: '/admin_dashboard.html', icon: 'admin_panel_settings', isStatic: true },
  ];
            ...
            {navLinks.map((link) => (
              link.isStatic ? (
                <a
                  key={link.name}
                  href={link.path}
                  className="text-white/40 font-bold text-[10px] uppercase tracking-[0.2em] hover:text-primary transition-all duration-300 font-label border-l border-white/10 pl-8 ml-2"
                >
                  {link.name}
                </a>
              ) : (
